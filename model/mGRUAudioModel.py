...
class mGRUAudioAttentionBeamsearchCaptionModel(object):
	...
	def __init__(self, input_feature, input_captions, input_audio, voc_size, d_w2v, output_dim, done_token=3, max_len = 20, beamsearch_batchsize = 1, beam_size=5, 
		...
		self.input_audio_dim = input_audio_dim*2
		

		self.input_audio = tf.reshape(input_audio,[-1,self.input_audio_dim])
		self.audio_embedding_dim = input_feature.get_shape().as_list()[-1]

		self.audio_matrix = self.init_audio_parameters(self.audio_embedding_dim)
		print(self.input_audio.get_shape().as_list())
		self.input_audio = tf.matmul(self.input_audio,self.audio_matrix)
		self.input_audio = tf.tile(tf.reshape(self.input_audio,[-1,1,self.audio_embedding_dim]),[1,input_feature.get_shape().as_list()[-2],1])
