...
class mGRUCategoriesAttentionCaptionModel(object):
	'''
		caption model for ablation studying
	'''
	def __init__(self, input_feature, input_captions, input_categories, voc_size, d_w2v, output_dim, 
		...
		self.cate_matrix = self.init_categories_matrix(input_feature.get_shape().as_list()[-1])
		self.input_categories_feature = tf.gather(self.cate_matrix,tf.tile(input_categories,[1,input_feature.get_shape().as_list()[1]]))

		self.input_feature = tf.concat([input_feature,self.input_categories_feature],-1)
...
class mGRUCategoriesAttentionBeamsearchCaptionModel(object):
	'''
		caption model for ablation studying
	'''
	def __init__(self, input_feature, input_captions, input_categories, voc_size, d_w2v, output_dim, done_token=3, max_len = 20, beamsearch_batchsize = 1, beam_size=5, 
		num_categories = 20, T_k=[1,3,6], attention_dim = 100, dropout=0.5,
		inner_activation='hard_sigmoid',activation='tanh',
		return_sequences=True):
...
class mGRUCategoriesAudioAttentionBeamsearchCaptionModel(object):
	'''
		caption model for ablation studying
	'''
	def __init__(self, input_feature, input_captions, input_categories, input_audio, voc_size, d_w2v, output_dim, done_token=3, max_len = 20, beamsearch_batchsize = 1, beam_size=5, 
		num_categories = 20, T_k=[1,3,6], attention_dim = 100, dropout=0.5,
		inner_activation='hard_sigmoid',activation='tanh',
		return_sequences=True):

		self.input_audio_dim = 34
		...
		self.input_feature = tf.concat([input_feature,self.input_categories_feature,self.input_audio],-1)
		print('input_feature.shape(),', self.input_feature.get_shape().as_list())
		self.input_captions = input_captions
