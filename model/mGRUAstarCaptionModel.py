...
class mGRUAstarAttentionCaptionModel(object):
	'''
		caption model for ablation studying
	'''
	def __init__(self, input_feature1, input_feature2, input_captions, voc_size, d_w2v, output_dim, T_k=[1,3,6], attention_dim = 100, dropout=0.5,
		inner_activation='hard_sigmoid',activation='tanh',
		return_sequences=True):
		self.input_feature1 = input_feature1
		self.input_feature2 = tf.transpose(input_feature2, perm=[0,1,3,4,2])
		print('input feature2 shape:', self.input_feature2)

		# self.input_feature2 = tf.reduce_sum(input_feature2,axis=[1])
		...
		self.encoder_input_shape1 = self.input_feature1.get_shape().as_list()
		self.encoder_input_shape2 = self.input_feature2.get_shape().as_list()
	...
	def init_parameters(self):
		...
		# encoder parameters
		# print(self.encoder_input_shape1)
		# encoder_i2h_shape = (self.encoder_input_shape1[-1],self.output_dim)
		...
		self.W_a = InitUtil.init_weight_variable((self.encoder_input_shape2[-1],self.attention_dim),init_method='glorot_uniform',name="W_a")
		...
		self.A_z = InitUtil.init_weight_variable((self.encoder_input_shape2[-1],self.output_dim),init_method='orthogonal',name="A_z")

		self.A_r = InitUtil.init_weight_variable((self.encoder_input_shape2[-1],self.output_dim),init_method='orthogonal',name="A_r")

		self.A_h = InitUtil.init_weight_variable((self.encoder_input_shape2[-1],self.output_dim),init_method='orthogonal',name="A_h")
	...
	# 	timesteps = self.encoder_input_shape1[1]

	# 	embedded_feature = self.input_feature1
	...
	# 	input_feature1 = tf.TensorArray(
	#             dtype=embedded_feature.dtype,
	#             size=timesteps,
	#             tensor_array_name='input_feature1')
	# 	if hasattr(input_feature1, 'unstack'):
	# 		input_feature1 = input_feature1.unstack(embedded_feature)
	# 	else:
	# 		input_feature1 = input_feature1.unpack(embedded_feature)	
	...
	# 		x_t = input_feature1.read(time) # batch_size * dim
	...
	# 	encoder_output = tf.reshape(encoder_output,[-1,self.encoder_input_shape1[1],self.output_dim])
	...
	def decoder(self, initial_state):
		...
		# self.input_feature2 = tf.transpose(self.input_feature2, perm=[0,2,3,1])

		def step(x_t,h_tm1):

			# ori_feature = tf.reshape(self.input_feature1,(-1,self.encoder_input_shape1[-1]))

			# attend_wx = tf.reshape(tf.nn.xw_plus_b(ori_feature, self.W_a, self.b_a),(-1,self.encoder_input_shape1[-2],self.attention_dim))
			# attend_uh_tm1 = tf.tile(tf.expand_dims(tf.matmul(h_tm1, self.U_a),dim=1),[1,self.encoder_input_shape1[-2],1])
			...
			# attend_e = tf.nn.softmax(tf.reshape(attend_e,(-1,self.encoder_input_shape1[-2],1)),dim=1)

			# attend_fea = self.input_feature1 * tf.tile(attend_e,[1,1,self.encoder_input_shape1[-1]])
			# attend_fea = tf.reduce_sum(attend_fea,reduction_indices=1)

			ori_feature = tf.reshape(self.input_feature2,(-1,self.encoder_input_shape2[-1]))

			attend_wx = tf.reshape(tf.nn.xw_plus_b(ori_feature, self.W_a, self.b_a),
				(-1,self.encoder_input_shape2[1]*self.encoder_input_shape2[2]*self.encoder_input_shape2[3],self.attention_dim))
			...
			attend_e = tf.nn.softmax(tf.reshape(attend_e,(-1,self.encoder_input_shape2[1]*self.encoder_input_shape2[2]*self.encoder_input_shape2[3],1)),dim=1)

			attend_fea = tf.multiply(tf.reshape(ori_feature,
					[-1,self.encoder_input_shape2[1]*self.encoder_input_shape2[2]*self.encoder_input_shape2[3],self.encoder_input_shape2[-1]]),
					attend_e)
			attend_fea = tf.reduce_sum(attend_fea,axis=1)
...
class mGRUMultiAstarAttentionCaptionModel(object):
	'''
		caption model for ablation studying
	'''
	def __init__(self, input_feature1, input_feature2, input_feature3, input_captions, voc_size, d_w2v, output_dim, attention_dim = 100, dropout=0.5,
		inner_activation='hard_sigmoid',activation='tanh',
		return_sequences=True):
		self.input_feature1 = input_feature1
		self.input_feature2 = tf.transpose(input_feature2, perm=[0,1,3,4,2])
		...
		self.input_feature3 = tf.transpose(input_feature3, perm=[0,1,3,4,5,2])

		self.W_conv4 = InitUtil.init_weight_variable((2,2,2,512,512),init_method='glorot_uniform',name="c3d_conv4_w")
		self.b_conv4 = InitUtil.init_bias_variable((512,),name="c3c_conv4_b")

		self.input_feature3 = self.conv3d('conv4',tf.reshape(self.input_feature3,[-1,2,7,7,512]),self.W_conv4,self.b_conv4)
		self.input_feature3 = tf.nn.relu(self.input_feature3, 'relu')
		self.input_feature3 = self.max_pool('pool', self.input_feature3, k=2)

		self.input_feature3 = tf.reshape(self.input_feature3,[-1,5,4,4,512])
		print('self.input_feature3.shape:',self.input_feature3.get_shape().as_list())


		self.input_feature2 = tf.concat([self.input_feature2,self.input_feature3],-1)
		# self.input_feature2 = tf.reduce_sum(input_feature2,axis=[1])
		self.input_captions = input_captions
		...
		self.encoder_input_shape1 = self.input_feature1.get_shape().as_list()
		self.encoder_input_shape2 = self.input_feature2.get_shape().as_list()
		self.encoder_input_shape3 = self.input_feature3.get_shape().as_list()
	...
	def init_parameters(self):
		...
		self.W_a_layer = InitUtil.init_weight_variable((self.encoder_input_shape3[-1],self.attention_dim),init_method='glorot_uniform',name="W_a_layer")
		self.U_a_layer = InitUtil.init_weight_variable((self.output_dim,self.attention_dim),init_method='orthogonal',name="U_a_layer")
		self.b_a_layer = InitUtil.init_bias_variable((self.attention_dim,),name="b_a_layer")

		self.W_layer = InitUtil.init_weight_variable((self.attention_dim,1),init_method='glorot_uniform',name="W_layer")


		self.A_z = InitUtil.init_weight_variable((self.encoder_input_shape3[-1],self.output_dim),init_method='orthogonal',name="A_z")
		self.A_r = InitUtil.init_weight_variable((self.encoder_input_shape3[-1],self.output_dim),init_method='orthogonal',name="A_r")
		self.A_h = InitUtil.init_weight_variable((self.encoder_input_shape3[-1],self.output_dim),init_method='orthogonal',name="A_h")
	...
	def decoder(self, initial_state):
		...
		def step(x_t,h_tm1):
			...
			##abtract attention
			attend_fea_layer = tf.reshape(attend_fea,[-1,self.encoder_input_shape3[-1]])
			attend_wx_layer = tf.reshape(tf.nn.xw_plus_b(attend_fea_layer, self.W_a_layer, self.b_a_layer),
				(-1,2,self.attention_dim))
			attend_uh_tm1_layer = tf.expand_dims(tf.matmul(h_tm1, self.U_a_layer),dim=1)
			attend_e_layer = tf.nn.tanh(tf.add(attend_wx_layer,attend_uh_tm1_layer))
			attend_e_layer = tf.matmul(tf.reshape(attend_e_layer,(-1,self.attention_dim)),self.W_layer)
			attend_e_layer = tf.nn.softmax(tf.reshape(attend_e_layer,(-1,2,1)),dim=1)

			attend_fea_layer = tf.multiply(tf.reshape(attend_fea_layer,
					[-1,2,self.encoder_input_shape3[-1]]),
					attend_e_layer)
			attend_fea_layer = tf.reduce_sum(attend_fea_layer,axis=1)
			...
			r = hard_sigmoid(preprocess_x_r+ tf.matmul(h_tm1, self.U_d_r) + tf.matmul(attend_fea_layer,self.A_r))
			z = hard_sigmoid(preprocess_x_z+ tf.matmul(h_tm1, self.U_d_z) + tf.matmul(attend_fea_layer,self.A_z))
			hh = tf.nn.tanh(preprocess_x_h+ tf.matmul(r*h_tm1, self.U_d_h) + tf.matmul(attend_fea_layer,self.A_h))
