...
def build_model(input_video, input_captions, 
			v2i,w2v,visual_pca_mat=None,d_lproj=300,d_w2v=512,
			labels = None, lr=0.01):
	with tf.variable_scope('share_embedding_matrix') as scope:
		T_B, T_w2v, T_mask = CaptionModelUtil.setWord2VecModelConfiguration(v2i,w2v,d_w2v,d_lproj=d_lproj)

		# encode question
		embeded_caption_words, mask_c = CaptionModelUtil.getDirectEmbeddingWithWord2Vec(input_captions, T_w2v, T_mask)
		embeded_video = CaptionModelUtil.getDirectVisualEncoder(input_video, T_w2v, visual_pca_mat=visual_pca_mat)

		# predict_score, predict_words = CaptionModelUtil.getDirectCaptionEncoderDecoder(embeded_video, embeded_caption_words, T_w2v, T_mask, mask_c)

		embeded_video = tf.nn.l2_normalize(embeded_video,-1)
		predict_score, predict_words = CaptionModelUtil.getDirectCaptionEncoderDecoderWithAttention(embeded_video, input_video, embeded_caption_words, T_w2v, T_mask)
...
def linear_project_pca_initialization(hf,  feature_shape, d_lproj=512, output_path=None):

	print('--utilize PCA to initialize the embedding matrix of feature to d_lproj')
	samples = []
...
def main(hf,f_type,capl=16,d_lproj=300,
	...
	w2v_googlenews_filename = '/home/xyj/usr/local/GoogleNewsW2V/new_w2v_caption_1024_iter100.bin'
	w2v_model = KeyedVectors.load_word2vec_format(w2v_googlenews_filename, binary=True)
	...
	if os.path.exists(visual_pca_mat_file):
		...
		visual_pca_mat = linear_project_pca_initialization(hf, feature_shape, d_lproj=d_lproj, output_path=visual_pca_mat_file)
	...
	train, loss, predict_words = build_model(input_video, input_captions, 
			v2i,w2v_model,visual_pca_mat=visual_pca_mat,d_lproj=d_lproj,d_w2v=1024,
			labels=y, lr=lr)
	...
	with sess.as_default():
		...
		for epoch in xrange(total_epoch):
			...
			#save model
			export_path = '/home/xyj/usr/local/saved_model/caption/direct_semantic'+'_'+f_type+'/'+'lr'+str(lr)+'_f'+str(feature_shape[0])
...
if __name__ == '__main__':
	...
	visual_pca_mat_file = '/home/xyj/usr/local/code/VideoQA/model/pca_mat/caption_google_pca_mat1024.pkl'
	...
	# pretrained_model = '/home/xyj/usr/local/saved_model/vqa_baseline/video+subtitle_classifier_semantic_HybridCNN/lr0.01_f16/E10_A0.36230248307'
	# pretrained_model = None
	
	main(hf,f_type,capl=13,d_lproj=1024,
