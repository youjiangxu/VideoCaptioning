...
class mGRUSecondOrderCaptionModel(object):
	'''
...
class mGRUFourOrderCaptionModel(object):
	'''
		caption model for ablation studying
	'''
	def __init__(self, input_feature, input_captions, voc_size, d_w2v, output_dim, done_token=3, max_len = 20, beamsearch_batchsize = 1, beam_size=5, 
	...
	def init_parameters(self):
		...
		self.U_a_4 = InitUtil.init_weight_variable((self.output_dim,self.attention_dim),init_method='orthogonal',name="U_a_4")
	...
	def decoder(self, initial_state):
		...
		def step(x_t,h_tm1,h_tm2,h_tm3,h_tm4):
			...
			attend_uh_tm4 = tf.tile(tf.expand_dims(tf.matmul(h_tm4, self.U_a_4),dim=1),[1,self.encoder_input_shape[-2],1])

			attend_e = tf.nn.tanh(attend_wx+attend_uh_tm1+attend_uh_tm2+attend_uh_tm3+attend_uh_tm4)
			attend_e = tf.matmul(tf.reshape(attend_e,(-1,self.attention_dim)),self.W)# batch_size * timestep
			# attend_e = tf.reshape(attend_e,(-1,attention_dim))
			attend_e = tf.nn.softmax(tf.reshape(attend_e,(-1,self.encoder_input_shape[-2],1)),dim=1)
		...
		def train_step(time, train_hidden_state, h_tm1, h_tm2, h_tm3, h_tm4):
			x_t = input_embedded_words.read(time) # batch_size * dim
			mask_t = input_mask.read(time)

			h = step(x_t, h_tm1, h_tm2, h_tm3, h_tm4)
			...
			return (time+1, train_hidden_state, h, h_tm1, h_tm2, h_tm3)
		...
		train_out = tf.while_loop(
	            cond=lambda time, *_: time < timesteps,
	            body=train_step,
	            loop_vars=(time, train_hidden_state, initial_state, initial_state, initial_state, initial_state),
	            parallel_iterations=32,
	            swap_memory=True)


		train_hidden_state = train_out[1]
		train_last_output = train_out[-4] 
		...
		def test_step(time, test_hidden_state, test_input_embedded_words, predict_words, h_tm1, h_tm2, h_tm3, h_tm4):
			x_t = test_input_embedded_words.read(time) # batch_size * dim
			...
			return (time+1,test_hidden_state, test_input_embedded_words, predict_words, h, h_tm1, h_tm2, h_tm3)
		...
		test_out = tf.while_loop(
	            cond=lambda time, *_: time < timesteps,
	            body=test_step,
	            loop_vars=(time, test_hidden_state, test_input_embedded_words, predict_words, initial_state, initial_state, initial_state, initial_state),
	            parallel_iterations=32,
	            swap_memory=True)


		predict_words = test_out[-5]
	...
	def beamSearchDecoder(self, initial_state):
		...
		def test_step(time, x_t, h_tm1, h_tm2, h_tm3, h_tm4, past_symbols, past_logprobs, finished_beams, logprobs_finished_beams):
			...
			return (time+1, symbols, h, h_tm1, h_tm2, h_tm3, past_symbols, past_logprobs, finished_beams, logprobs_finished_beams)
	 ...
	 #            clear_after_read=None)
		...
		test_out = tf.while_loop(
	            cond=lambda time, *_: time < timesteps,
	            body=test_step,
	            loop_vars=(time, symbols, h, h_0, h_0, h_0, past_symbols, past_logprobs, finished_beams, logprobs_finished_beams),
	            parallel_iterations=32,
	            swap_memory=True)
