...
def build_model(input_video, input_captions, 
			v2i,visual_pca_mat=None,d_lproj=300,
			labels = None, lr=0.01):
	with tf.variable_scope('share_embedding_matrix') as scope:
		...
		predict_score, predict_words = CaptionModelUtil.getDirectCaptionGRUEncoderDecoder(embeded_video, embeded_caption_words, T_w2v, T_mask, mask_c)
...
def main(hf,f_type,capl=16,d_lproj=300,
	...
	# Create vocabulary
	v2i, train_data, val_data, test_data = CaptionDataUtil.create_vocabulary_word2vec(file, capl=12, v2i={'': 0, 'UNK':1,'BOS':2, 'EOS':3})
	...
	train, loss, predict_words = build_model(input_video, input_captions, 
			v2i,visual_pca_mat=visual_pca_mat,d_lproj=d_lproj,
			labels=y, lr=lr)
	...
	with sess.as_default():
		...
		for epoch in xrange(total_epoch):
			...
			#save model
			export_path = '/home/xyj/usr/local/saved_model/caption/gru_direct_semantic'+'_'+f_type+'/'+'lr'+str(lr)+'_f'+str(feature_shape[0])
...
if __name__ == '__main__':
	...
	visual_pca_mat_file = '/home/xyj/usr/local/code/VideoQA/model/direct_pca_mat/caption_google_pca_mat.pkl'
	...
	main(hf,f_type,capl=16,d_lproj=512,
