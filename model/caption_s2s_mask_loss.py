...
def build_model(input_video, input_captions, 
	...
	with tf.variable_scope('share_embedding_matrix') as scope:
		...
		# encode question
		embeded_caption_words, mask_c = CaptionModelUtil.getDirectEmbeddingWithWord2VecWithIntMask(input_captions, T_w2v, T_mask)


		# predict_score, predict_words = CaptionModelUtil.getSequence2SequenceCaption(input_video, embeded_caption_words, T_w2v, T_mask)

		train_loss, predict_words = CaptionModelUtil.getS2SCaptionWithAttentionWithMaskLoss(input_video, embeded_caption_words, T_w2v, T_mask, mask_c, labels)
		...
		return train, train_loss, predict_words
...
if __name__ == '__main__':
	...
	---------------------------------
	'''
	hf = h5py.File(feature_path,'r', driver='core')

	pretrained_model = None
	
	main(hf,f_type,capl=12,d_lproj=512,
