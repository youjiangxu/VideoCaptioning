...
class ConvMemoryAttentionModel(object):
	'''
		caption model for ablation studying
		output_dim = num_of_filter
	'''
	def __init__(self, input_feature, input_captions, voc_size, d_w2v, output_dim, 
		memory_size=32, mem_ker_size=3,
		filter_height=1, filter_width=1, stride=[1,1,1,1], pad='SAME', 
		done_token=3, max_len = 20, beamsearch_batchsize = 1, beam_size=5,
		attention_dim = 100, dropout=0.5,
		inner_activation='hard_sigmoid',activation='tanh',
		...
		self.mem_ker_size = mem_ker_size
		self.memory_size = mem_ker_size
	...
	def init_parameters(self):
		...
		self.A = InitUtil.init_weight_variable((self.output_dim,3*self.output_dim),init_method='glorot_uniform',name="A")

		


		# glu parameters
		memory_kernel_shape = (self.mem_ker_size, self.mem_ker_size, self.output_dim, 2*self.memory_size)
		self.W_e_m = InitUtil.init_weight_variable(memory_kernel_shape,init_method='glorot_uniform',name="W_e_m")
		self.b_e_m = InitUtil.init_bias_variable((2*self.memory_size,),name="b_e_m")

		self.video_memory_size = self.encoder_input_shape[1]*self.memory_size

		self.W_ma = InitUtil.init_weight_variable((self.mem_ker_size, self.mem_ker_size, self.video_memory_size, self.attention_dim),init_method='glorot_uniform',name="W_ma")
		self.b_ma = InitUtil.init_bias_variable((self.attention_dim,),name="b_ma")

		self.Wm = InitUtil.init_weight_variable((self.attention_dim,1),init_method='glorot_uniform',name="Wm")

		self.A_m = InitUtil.init_weight_variable((self.video_memory_size,3*self.output_dim),init_method='glorot_uniform',name="A_m")
	...
	def encoder(self):
		...
		conv_memory_states = tf.TensorArray(
	            dtype=tf.float32,
	            size=timesteps,
	            tensor_array_name='conv_memory_states')
		
		def feature_step(time, hidden_states, conv_memory_states, h_tm1):
			preprocess_x = input_feature.read(time) # batch_size * dim
			# # (batch, height, width, channels)
			...
			cm = tf.nn.conv2d(h,self.W_e_m,self.stride,self.pad,name='to_memory') # conv memory
			cm = tf.add(cm,tf.reshape(self.b_e_m,[-1, 1, 1, 2*self.memory_size]))
			cm_a = cm[:,:,:,0:self.memory_size]
			cm_b = cm[:,:,:,self.memory_size::]
			cm_t = cm_a*tf.nn.tanh(cm_b)
			conv_memory_states = conv_memory_states.write(time,cm_t)

			return (time+1,hidden_states,conv_memory_states, h)
		...
		feature_out = tf.while_loop(
	            cond=lambda time, *_: time < timesteps,
	            body=feature_step,
	            loop_vars=(time, hidden_states, conv_memory_states, initial_state ),
	            parallel_iterations=32,
	            swap_memory=True)

		last_output = feature_out[-1] 
		conv_memory_states = feature_out[-2]
		hidden_states = feature_out[-3]
		if hasattr(hidden_states, 'stack'):
			encoder_output = hidden_states.stack()
		else:
			encoder_output = hidden_states.pack()
		...
		if hasattr(conv_memory_states, 'stack'):
			conv_memory = conv_memory_states.stack()
		else:
			conv_memory = conv_memory_states.pack()
		axis = [1,2,3,0,4]  # axis = [1,0,2]
		conv_memory = tf.transpose(conv_memory, perm=axis)
		conv_memory = tf.reshape(conv_memory,[-1, self.encoder_input_shape[2], self.encoder_input_shape[3], self.encoder_input_shape[1]*self.memory_size])
		# conv_memory = tf.concat(conv_memory,axis=-1)
		print('conv_memory_shape,',conv_memory.get_shape().as_list())

		return last_output, encoder_output, conv_memory
	...
	def decoder(self, initial_state, input_feature, conv_memory):
		...
		def step(x_t,h_tm1):
			...
			attend_wx = tf.reshape(tf.nn.xw_plus_b(ori_feature, self.W_a, self.b_a),(-1,self.encoder_input_shape[1],self.attention_dim))
			attend_uh_tm1 = tf.matmul(h_tm1, self.U_a)

			attend_e = tf.nn.tanh(tf.add(attend_wx,tf.expand_dims(attend_uh_tm1,dim=1)))
			attend_e = tf.matmul(tf.reshape(attend_e,(-1,self.attention_dim)),self.W) # batch_size * timestep
			attend_e = tf.nn.softmax(tf.reshape(attend_e,(-1,self.encoder_input_shape[1],1)),dim=1)
			...
			# batch, height, width, time*mem_size
			mem_wx = tf.add(tf.nn.conv2d(conv_memory,self.W_ma, self.stride, self.pad, name='mem_att_wx'),tf.reshape(self.b_ma,[1,1,1,self.attention_dim]))
			attend_m = tf.nn.tanh(tf.add(mem_wx,tf.reshape(attend_uh_tm1,[-1,1,1,self.attention_dim])))

			attend_m = tf.matmul(tf.reshape(attend_m,[-1,self.attention_dim]), self.Wm)
			attend_m = tf.nn.softmax(tf.reshape(attend_m,(-1,self.encoder_input_shape[2]*self.encoder_input_shape[3],1)),dim=1)

			attend_fea_m = tf.multiply(conv_memory, tf.reshape(attend_m,[-1,self.encoder_input_shape[2],self.encoder_input_shape[3],1]))
			attend_fea_m = tf.reduce_sum(attend_fea_m, reduction_indices=[1,2])

			attend_fea_m = tf.matmul(attend_fea_m,self.A_m)
			print('attend_fea_m',attend_fea_m.get_shape().as_list())

			attend_fea_m_r = attend_fea_m[:,0:self.output_dim]
			attend_fea_m_z = attend_fea_m[:,self.output_dim:2*self.output_dim]
			attend_fea_m_h = attend_fea_m[:,2*self.output_dim::]
			...
			r = hard_sigmoid(preprocess_x_r+ tf.matmul(h_tm1, self.U_d_r) + attend_fea_r + attend_fea_m_r)
			z = hard_sigmoid(preprocess_x_z+ tf.matmul(h_tm1, self.U_d_z) + attend_fea_z + attend_fea_m_z)
			hh = tf.nn.tanh(preprocess_x_h+ tf.matmul(r*h_tm1, self.U_d_h) + attend_fea_h + attend_fea_m_h)
	...
	def beamSearchDecoder(self, initial_state_h, input_feature, conv_memory):
		'''
			captions: (batch_size x timesteps) ,int32
			d_w2v: dimension of word 2 vector
		'''
		def step(x_t,h_tm1):
			...
			# attend_fea = tf.reshape(tf.tile(tf.expand_dims(attend_fea,dim=1),[1,self.beam_size,1]),[self.batch_size*self.beam_size,3*self.output_dim])
			...
			attend_fea_m = tf.matmul(attend_fea_m,self.A_m)
			print('attend_fea_m',attend_fea_m.get_shape().as_list())
			# attend_fea_m = tf.reshape(tf.tile(tf.expand_dims(attend_fea_m,dim=1),[1,self.beam_size,1]),[self.batch_size*self.beam_size,3*self.output_dim])
	...
	def build_model(self):
		print('building model ... ...')
		self.init_parameters()
		last_output, encoder_output, conv_memory = self.encoder()

		predict_score, predict_words , loss_mask= self.decoder(last_output, encoder_output, conv_memory)
		finished_beam, logprobs_finished_beams, past_symbols = self.beamSearchDecoder(last_output, encoder_output, conv_memory)
		return predict_score, predict_words, loss_mask, finished_beam, logprobs_finished_beams, past_symbols
...
class SeqVladWithGLUAttentionModel(NetVladAttentionModel):
	'''
		caption model for ablation studying
		output_dim = num_of_filter
	'''
	...
	def init_parameters(self):
		...
		self.W_e = tf.get_variable("W_e", shape=[1, 1, self.enc_in_shape[-1], 2*self.centers_num], initializer=tf.contrib.layers.xavier_initializer())
		self.b_e = tf.get_variable("b_e",initializer=tf.random_normal([2*self.centers_num],stddev=1./math.sqrt(self.centers_num)))
		self.centers = tf.get_variable("centers",[1, 1, 1, self.enc_in_shape[-1], self.centers_num],
			initializer=tf.random_normal_initializer(stddev=1. / math.sqrt(self.enc_in_shape[-1])))
		...
		encoder_h2h_shape = (self.filter_size, self.filter_size, self.centers_num, 2*self.centers_num)
		self.U_e_r = tf.get_variable("U_e_r", shape=encoder_h2h_shape, initializer=tf.contrib.layers.xavier_initializer())
		self.U_e_z = tf.get_variable("U_e_z", shape=encoder_h2h_shape, initializer=tf.contrib.layers.xavier_initializer())
		self.U_e_h = tf.get_variable("U_e_h", shape=encoder_h2h_shape, initializer=tf.contrib.layers.xavier_initializer()) 
	...
	def encoder(self):
		...
		pre_assignment = tf.reshape(embedded_feature,[-1,self.enc_in_shape[2],self.enc_in_shape[3],self.enc_in_shape[4]])
		pre_assignment = tf.add(tf.nn.conv2d(pre_assignment, self.W_e, self.stride, self.pad, name='w_conv_x'),tf.reshape(self.b_e,[1, 1, 1, 2*self.centers_num]))
		
		pre_assignment = tf.reshape(pre_assignment,[-1,self.enc_in_shape[1],self.enc_in_shape[2],self.enc_in_shape[3],2*self.centers_num])

		assignment_a = pre_assignment[:,:,:,:,0:self.centers_num]
		assignment_b = pre_assignment[:,:,:,:,self.centers_num::]

		assignment = assignment_a*tf.nn.tanh(assignment_b)
		...
		def step(time, hidden_states, h_tm1):
			...
			ur = tf.nn.conv2d(h_tm1, self.U_e_r, self.stride, self.pad, name='ur')
			ur_a =  ur[:,:,:,0:self.centers_num]
			ur_b = ur[:,:,:,self.centers_num::]
			ur_rr = ur_a*tf.nn.tanh(ur_b)

			uz = tf.nn.conv2d(h_tm1, self.U_e_z, self.stride, self.pad, name='uz')
			uz_a =  uz[:,:,:,0:self.centers_num]
			uz_b = uz[:,:,:,self.centers_num::]
			uz_zz = uz_a*tf.nn.tanh(uz_b)
			
			r = hard_sigmoid(assign_t+ ur_rr)
			z = hard_sigmoid(assign_t+ uz_zz)
			
			uh = tf.nn.conv2d(r*h_tm1, self.U_e_h, self.stride, self.pad, name='uh_hh')
			uh_a =  uh[:,:,:,0:self.centers_num]
			uh_b = uh[:,:,:,self.centers_num::]
			uh_hh = uh_a*tf.nn.tanh(uh_b)

			hh = tf.tanh(assign_t+ uh_hh)
