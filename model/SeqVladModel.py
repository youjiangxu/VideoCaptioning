...
# class SeqVladConvAttentionModel(NetVladAttentionModel):
# 	'''
# 		caption model for ablation studying
# 		output_dim = num_of_filter
# 	'''
...
# 		# self.W_e = InitUtil.init_weight_variable(encoder_i2h_shape,init_method='glorot_uniform',name="W_e")
# 		# self.b_e = InitUtil.init_bias_variable((self.centers_num,),name="b_e")

# 		# self.alpha = tf.Variable(10,name='alpha')
# 		# self.centers = tf.reshape(tf.cast(self.alpha,tf.float32)*self.W_e,[1, 1, 1, self.enc_in_shape[-1], self.centers_num])
# 		self.init_centers = tf.cast(tf.reshape(tf.transpose(self.init_centers,perm=[1,0]),[-1, self.enc_in_shape[-1], self.centers_num]),tf.float32)
# 		self.centers = tf.Variable(self.init_centers,dtype=tf.float32, name='centers')
...
# 		self.liner_W = InitUtil.init_weight_variable((self.enc_in_shape[-1], self.output_dim),init_method='glorot_uniform',name="liner_W")
# 		self.liner_b = InitUtil.init_bias_variable((self.output_dim,),name="liner_b")
...
# 		self.W_a = InitUtil.init_weight_variable((self.enc_in_shape[-1]*self.centers_num,self.attention_dim),init_method='glorot_uniform',name="W_a")
...
# 		self.A = InitUtil.init_weight_variable((self.enc_in_shape[-1]*self.centers_num,3*self.output_dim),init_method='glorot_uniform',name="A")
...
# 		train_input_feature = tf.nn.dropout(input_feature,self.dropout)
# 		test_input_feature = self.dropout*input_feature
# 		def step(x_t,h_tm1,input_feature):
...
# 			h = step(x_t,h_tm1,train_input_feature)
...
# 			h = step(x_t,h_tm1,test_input_feature)
