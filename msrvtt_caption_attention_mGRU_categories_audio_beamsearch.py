...
def exe_train(sess, data, audio_info, cate_info, batch_size, v2i, hf, feature_shape, 
	train, loss, input_video, input_captions, input_categories, input_audio, y, capl=16):
	...
	# for batch_idx in xrange(500):
		...
		data_audio = MsrDataUtil.getBatchVideoAudioInfo(batch_caption, audio_info, feature_shape)

		_, l = sess.run([train,loss],feed_dict={input_video:data_v, input_captions:data_c, input_categories:data_cate, input_audio:data_audio, y:data_y})
		total_loss += l
		print('    batch_idx:%d/%d, loss:%.5f' %(batch_idx+1,num_batch,l))
	total_loss = total_loss/num_batch
	return total_loss

def exe_test(sess, data, audio_info, cate_info, batch_size, v2i, i2v, hf, feature_shape, 
	predict_words, input_video, input_captions, input_categories, input_audio, y, capl=16):
	...
	for batch_idx in xrange(num_batch):
		...
		[gw] = sess.run([predict_words],feed_dict={input_video:data_v, input_captions:data_c, input_categories:data_cate, input_audio:data_audio, y:data_y})
...
def beam_search_exe_test(sess, data, audio_info, cate_info, batch_size, v2i, i2v, hf, feature_shape, 
	predict_words, input_video, input_captions, input_categories, input_audio, y, finished_beam, logprobs_finished_beams, past_logprobs, capl=16):
	...
	for batch_idx in xrange(num_batch):
		...
		[gw,tw,gp, gl] = sess.run([finished_beam, predict_words, logprobs_finished_beams, past_logprobs],
			                  feed_dict={input_video:data_v, input_captions:data_c, input_categories:data_cate, input_audio:data_audio, y:data_y})
...
def main(hf, audio_info, f_type,capl=16, d_w2v=512, output_dim=512,
	...
	input_audio = tf.placeholder(tf.float32, shape=(None,34,40), name='input_audio')
	y = tf.placeholder(tf.int32,shape=(None, capl))

	attentionCaptionModel = mGRUCaptionCategoriesModel.mGRUCategoriesAudioAttentionBeamsearchCaptionModel(input_video, input_captions, input_categories, input_audio, voc_size, d_w2v, output_dim, 
		T_k=[1,2,4,8], max_len = 20, beamsearch_batchsize = 1, beam_size=5)
	...
	with sess.as_default():
		...
		for epoch in xrange(total_epoch):
			...
			# # train phase
			tic = time.time()
			total_loss = exe_train(sess, train_data, audio_info, cate_info, batch_size, v2i, hf, feature_shape, train, loss, input_video, input_captions, input_categories, input_audio, y, capl=capl)

			print('    --Train--, Loss: %.5f, .......Time:%.3f' %(total_loss,time.time()-tic))

			tic = time.time()
			js = exe_test(sess, test_data, audio_info, cate_info, batch_size, v2i, i2v, hf, feature_shape, 
										predict_words, input_video, input_captions, input_categories, input_audio, y, capl=capl)
			print('    --Val--, .......Time:%.3f' %(time.time()-tic))

			# tic = time.time()
			

			# js = beam_search_exe_test(sess, test_data, audio_info, cate_info, 1, v2i, i2v, hf, feature_shape, 
			# 							predict_words, input_video, input_captions, input_categories, input_audio, y, finished_beam, logprobs_finished_beams, past_logprobs, capl=capl)
...
if __name__ == '__main__':
	...
	f_type = 'audio_categories_sparse_mgru1248_attention_resnet152_dw2v'+str(d_w2v)+'_outputdim'+str(output_dim)
	...
	audio_info = np.load('/home/xyj/usr/local/data/msrvtt/audio2feats_channel1_new.npy')
