...
from model import TestCaptionModel 
...
def exe_test(sess, data, cate_info, batch_size, v2i, i2v,  hf1, hf2, feature_shape1, feature_shape2, 
	predict_words, input_video1, input_video2, input_captions, input_categories, y, capl=16, version=1):
...
def beamsearch_exe_test(sess, data, cate_info, batch_size, v2i, i2v,  hf1, hf2, feature_shape1, feature_shape2, 
	predict_words, input_video1, input_video2, input_captions, input_categories, y, finished_beam, logprobs_finished_beams, capl=16, version=2):
...
def main(hf1,hf2,f_type,num_training=None, capl=16, d_w2v=512, output_dim=512,
	...
	# Create vocabulary
	v2i, train_data, test_data = MsrFinalDataUtil.create_vocabulary_word2vec(file, capl=capl, word_threshold=1, 
						v2i={'': 0, 'UNK':1,'BOS':2, 'EOS':3}, num_training=num_training)
	...
	attentionCaptionModel = TestCaptionModel.TestModel(input_video1, input_video2, input_captions, input_categories, voc_size, d_w2v, output_dim, 
	...
	with sess.as_default():
		...
		if Test is not None:
			...
			test_js = exe_test(sess, test_data, cate_info, batch_size, v2i, i2v, hf1, hf2, feature_shape1, feature_shape2, 
										predict_words, input_video1, input_video2, input_captions, input_categories, y, capl=capl, version=1)

			with open(result_path+'/result1(primary).json', 'w') as f:
				json.dump(test_js, f)
			...
			test_js = beamsearch_exe_test(sess, test_data, cate_info, 1, v2i, i2v, hf1, hf2, feature_shape1, feature_shape2, 
											...
											capl=capl,version=2)
			with open(result_path+'/result2.json', 'w') as f:
				json.dump(test_js, f)
...
if __name__ == '__main__':
	...
	f_type = 'tese_'+str(num_training)+'training_categories+resnet152+c3dfc7New_mLSTM1248_attention_dw2v'+str(d_w2v)+'_outputdim'+str(output_dim)
	...
	hf1 = h5py.File(feature_path1,'r')['images']
	hf2 = h5py.File(feature_path2,'r')['images']
	pretrained_model = '/home/xyj/usr/local/saved_model/msrvtt2017/tese_9000training_categories+resnet152+c3dfc7New_mLSTM1248_attention_dw2v512_outputdim512/lr0.0001_f40_B128/model/E1_L5.08920303631.ckpt'
	
	main(hf1,hf2,f_type,num_training=num_training, capl=capl, d_w2v=d_w2v, output_dim=output_dim,
		feature_shape1=feature_shape1, feature_shape2=feature_shape2, lr=lr,
		batch_size=batch_size,total_epoch=total_epoch,
		file='/home/xyj/usr/local/data/msrvtt',pretrained_model=pretrained_model,Test=None)
